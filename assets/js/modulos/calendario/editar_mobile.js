    ...
    try {
        $(id_calendario).fullCalendar({
            ...
            eventClick: function (event, jsEvent, view) {
                ...
                obtenerDialogoActividad_mobile(cargarEventos(event.id), event);
...
function obtenerDialogoActividad_mobile(datos, event) {
    if (datos.id > 0) {
        bootbox.dialog({
            title: (datos.id >= 0) ? s3vars.L_MOD.lbl_editar_actividad : s3vars.L_MOD.lbl_nueva_actividad,
            message: obtenerFormularioActividad_mobile(datos), //data
            buttons: {
                success: {
